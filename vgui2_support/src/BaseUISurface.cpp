...
void BaseUISurface::DrawUnicodeChar(wchar_t ch) {
	...
	gEngfuncs.pfnVGUI2DrawCharacterAdd(_drawTextPos[0] + _translateX, _drawTextPos[1] + _translateY, ch, _drawTextColor[0], _drawTextColor[1], _drawTextColor[2], m_hCurrentFont);
}
...
void BaseUISurface::GetCharABCwide(vgui2::HFont font, int ch, int &a, int &b, int &c) {
	// TODO: support custom fonts
	a = _screeninfo.charWidths[g_api->ProcessUtfChar(ch)];
	b = c = 0;
}

int BaseUISurface::GetCharacterWidth(vgui2::HFont font, int ch) {
	// TODO: support custom fonts
	return _screeninfo.charWidths[g_api->ProcessUtfChar(ch)];
}
...
void BaseUISurface::GetTextSize(vgui2::HFont, const wchar_t *text, int &wide, int &tall) {
	...
	for (; *text != 0; ++text) {
		wide += _screeninfo.charWidths[g_api->ProcessUtfChar(*text)];
	}
}
